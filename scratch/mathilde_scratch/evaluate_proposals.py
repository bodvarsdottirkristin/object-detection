...
class ProposalEvaluator:
    ...
    def __init__(self, dataset_path, proposals_dir, output_dir):
        ...
        print("✓ Initialized ProposalEvaluator")
    ...
    def evaluate_split(self, split="train", iou_threshold=0.5):
        ...
        results = []
        for image_filename, data in tqdm(
            proposals_dict.items(), desc=f"{split} - Evaluating"
        ):
            eval_result = self.evaluate_image(
                image_filename, data["proposals"], iou_threshold
            )
            if eval_result:
    ...
    def print_statistics(self, stats):
        ...
        print(
            f"Total positive proposals (IoU > 0.5): {stats['total_positive_proposals']}"
        )
        ...
        print("\nProposal efficiency:")
        ratio = (
            stats["total_positive_proposals"] / stats["total_proposals"]
    ...
    def plot_results(self, all_results):
        ...
        axes[1].set_title(
            "Proposal Quality (Avg IoU) by Split", fontsize=12, fontweight="bold"
        )
    ...
    def analyze_proposal_counts(self, all_results):
        ...
        for split, stats in all_results.items():
            ...
            print(
                f"  - Recommendation: Keep top {int(avg_props * 0.5):.0f} proposals (50%)"
            )
            print(
                f"                   or top {int(avg_props * 0.3):.0f} proposals (30%)"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_to_save = {}
        for split, stats in all_results.items():
            results_to_save[split] = {k: v for k, v in stats.items() if k != "details"}
